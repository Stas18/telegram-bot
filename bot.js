...
const coreFunctions = {
  uploadHistoryToGoogleSheets: async () => {
    try {
      ...
      if (history.length === 0) {
        logger.log('История пуста, нечего загружать в Google Sheets');
        return;
      }
      ...
      logger.log('✅ История успешно загружена в Google Sheets!');
    } catch (error) {
      logger.error(error, 'uploadHistoryToGoogleSheets');
      throw error;
    }
  },
  ...
  handleAdminCallbacks: async (query) => {
    ...
    const adminHandlers = {
      ...
      admin_save_to_history: async () => {
        ...
        // Очистка данных голосования перед сохранением
        votingManager.save({
          ratings: {},
          average: null,
          film: null,
        ...
        // Очистка данных о следующей встрече
        meetingManager.save(DEFAULT_MEETING);
